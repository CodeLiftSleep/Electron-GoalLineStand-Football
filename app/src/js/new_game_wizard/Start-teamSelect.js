        ...
        .controller('teamSelectCtrl', ['$scope', '$stateParams', '$uibModal', 'DB', 'uiGridConstants',
            function teamSelectCtrl($scope, $stateParams, $uibModal, DB, uiGridConstants) {
                    ...
                    var modalInstance = $uibModal.open({
                       ...
                       `,
                        controller: function($scope, $uibModalInstance, $interval, $filter) {
                            ...
                            $scope.team = $filter('filter')(DB.load.data.Teams, {
                                TeamID: teamId
                            }, true)[0];
                            $scope.teamPri = {
                                'background-color': $scope.team.MainColor,
                                'color': $scope.team.TrimColor
                            };
                            $scope.teamSec = {
                                'background-color': $scope.team.SecondaryColor
                            };
                            $scope.teamTer = {
                                'background-color': $scope.team.TrimColor
                            };
                ...
                vm.TeamInfo = function(teamSelected, teamId) {
                    ...
                    var modalInstance = $uibModal.open({
                        templateUrl: `../html/Start/team_info.html`,
                        resolve: {
                            coaches: coaches.push(_.filter(DB.load.data.Coaches, {
                                'TeamID': teamId
                            })),
                            personnel: personnel.push(_.filter(DB.load.data.Personnel, {
                                'TeamID': teamId
                            }))
                        ...
                        controller: function($scope, $uibModalInstance, $interval, $filter, uiGridConstants) {
                            ...
                            $scope.owner = _.filter(DB.load.data.Owners, {
                                'TeamID': teamId
                            })[0];
                            ...
                            $scope.teamOffYds = $filter('filter')(DB.load.data.TeamOffense, {
                                TeamID: teamId
                            }, true)[0].TotalYards / 16;
                            $scope.teamOffRank = _.findIndex(DB.load.data.TeamOffense, {
                                'TeamID': teamId
                            }) + 1; //lodash function
                            $scope.passOff = _.orderBy(DB.load.data.TeamOffense, ['PassingYards'], ['desc']);
                            $scope.passOffYds = _.filter(DB.load.data.TeamOffense, {
                                TeamID: teamId
                            })[0].PassingYards / 16;
                            $scope.passOffRank = _.findIndex($scope.passOff, {
                                'TeamID': teamId
                            }) + 1;
                            $scope.rushOff = _.orderBy(DB.load.data.TeamOffense, ['RushingYards'], ['desc']);
                            $scope.rushOffYds = _.filter(DB.load.data.TeamOffense, {
                                'TeamID': teamId
                            })[0].RushingYards / 16;
                            $scope.rushOffRank = _.findIndex($scope.rushOff, {
                                'TeamID': teamId
                            }) + 1;
                            $scope.pointsOff = _.orderBy(DB.load.data.TeamOffense, ['PointsFor'], ['desc']);
                            $scope.teamPointsOff = _.filter(DB.load.data.TeamOffense, {
                                'TeamID': teamId
                            })[0].PointsFor / 16;
                            $scope.pointsOffRank = _.findIndex($scope.pointsOff, {
                                'TeamID': teamId
                            }) + 1;
                            $scope.teamDefYds = $filter('filter')(DB.load.data.TeamDefense, {
                                TeamID: teamId
                            }, true)[0].TotalYards / 16;
                            $scope.teamDefRank = _.findIndex(DB.load.data.TeamDefense, {
                                'TeamID': teamId
                            }) + 1; //Lodash function
                            $scope.passDef = _.orderBy(DB.load.data.TeamDefense, ['PassingYards'], ['asc']);
                            $scope.passDefYds = _.filter(DB.load.data.TeamDefense, {
                                TeamID: teamId
                            })[0].PassingYards / 16;
                            $scope.passDefRank = _.findIndex($scope.passDef, {
                                'TeamID': teamId
                            }) + 1;
                            $scope.rushDef = _.orderBy(DB.load.data.TeamDefense, ['RushingYards'], ['asc']);
                            $scope.rushDefYds = _.filter(DB.load.data.TeamDefense, {
                                'TeamID': teamId
                            })[0].RushingYards / 16;
                            $scope.rushDefRank = _.findIndex($scope.rushDef, {
                                'TeamID': teamId
                            }) + 1;
                            $scope.pointsDef = _.orderBy(DB.load.data.TeamDefense, ['PointsAgainst'], ['asc']);
                            $scope.teamPointsDef = _.filter(DB.load.data.TeamDefense, {
                                'TeamID': teamId
                            })[0].PointsAllowed / 16;
                            $scope.pointsDefRank = _.findIndex($scope.pointsDef, {
                                'TeamID': teamId
                            }) + 1;
                            $scope.draftPos = DB.getNumEnding($scope.team.DraftPosition);
                            $scope.roster = _.filter(DB.load.data.RosterPlayers, {
                                'TeamID': teamId
                            });
               ...
               function getDraftPos(roster) { //get biggest weakness by checking player grades at each position and taking the average
                    ...
                    _.each($scope.roster, function(value) {
                        ...
                        switch(value.Pos) { //get the
                            ...
                            $scope.headCoach = _.find($scope.coaches, {
                                'CoachType': 1
                            });
                            var value;
                            ...
                            $scope.gridOptions1 = {
                                ...
                                columnDefs: [{
                                    ...
                                    },
                                    {
                                        ...
                                        displayName: 'Exp',
                                        width: '4%',
                                        cellTooltip: 'Years of NFL coaching experience'
                                    },
                                    {
                                        name: 'CoachType',
                                        visible: false,
                                    ...
                                    },
                                    {
                                        ...
                                        displayName: 'Spec.',
                                        width: '7%',
                                        cellTooltip: 'Side of ball the coach is most familiar with.'
                                    },
                                    {
                                        name: 'OffAbility',
                                        displayName: 'Off Ability',
                                        width: '7%',
                                        cellTooltip: 'Skill level at coaching offense.'
                                    },
                                    {
                                        name: 'OffPhil',
                                        displayName: 'Off Phil.',
                                        width: '7%',
                                        cellTooltip: 'Base offensive philosophy for the coach.'
                                    },
                                    {
                                        name: 'DefAbility',
                                        displayName: 'Def Ability',
                                        width: '7%',
                                        cellTooltip: 'Skill level at coaching defense.'
                                    },
                                    {
                                        name: 'DefPhil',
                                        displayName: 'Def Phil.',
                                        width: '7%',
                                        cellTooltip: 'Base defensive philosophy for the coach.'
                                    },
                                    {
                                        name: 'CoachAbility',
                                        displayName: 'Coach Ability',
                                        width: '8%',
                            ...
                            $scope.gridOptions2 = {
                                ...
                                columnDefs: [{
                                    ...
                                    },
                                    {
                                        ...
                                        displayName: 'Exp',
                                        width: '4%',
                                        cellTooltip: 'Years of NFL personnel experience'
                                    },
                                    {
                                        name: 'PersonnelType',
                                        visible: false,
                                    ...
                                    {
                                        name: 'PersonnelTypeStr',
                                        displayName: 'Position',
                                        width: '12%'
                                    },
                                    {
                                        name: 'ScoutRegion',
                                        displayName: 'Scout Region',
                                        width: '7%',
                                        cellTooltip: 'Scouting region or assignment they are responsible for.'
                                    },
                                    {
                                        name: 'DraftStrategy',
                                        displayName: 'Draft Strat',
                                        width: '8%',
                                        cellTooltip: 'Preferred strategy when it comes to the draft and drafting players.'
                                    },
                                    {
                                        name: 'TeamBuilding',
                                        displayName: 'Team Bldg',
                                        width: '8%',
                                        cellToolTip: 'Preferred strategy when it comes to building their team.'
                                    },
                                    {
                                        name: 'ValuesDraftPicks',
                                        displayName: 'Val Draft Picks',
                                        width: '7%',
                                        cellTooltip: 'How much they value draft picks when grading players.'
                                    },
                                    {
                                        name: 'ValuesProduction',
                                        displayName: 'Val Production',
                                        width: '7%',
                                        cellTooltip: 'How much they value production when grading players.'
                                    },
                                    {
                                        name: 'ValuesIntangibles',
                                        displayName: 'Val Intangibles',
                                        width: '7%',
                                        cellTooltip: 'How much they value intangibles when grading players.'
                                    },
                                    {
                                        name: 'ValuesCombine',
                                        displayName: 'Val Combine',
                                        width: '7%',
                                        cellTooltip: 'How much they value the combine when grading players.'
                                    },
                                    {
                                        name: 'ValuesCharacter',
                                        displayName: 'Val Character',
                                        width: '7%',
                                        cellTooltip: 'How much they value a player\'s character when grading players.'
                                    },
                                    {
                                        name: 'JudgingDraft',
                                        displayName: 'Judg. Draft',
                                        width: '7%',
                                        cellTooltip: 'How good they are at judging players in the draft.'
                                    },
                                    {
                                        name: 'JudgingFA',
                                        displayName: 'Judg. FA',
                                        width: '7%',
                                        cellTooltip: 'How good they are at judging free agents'
                                    },
                                    {
                                        name: 'JudgingOwn',
                                        displayName: 'Judg. Own',
                                        width: '7%',
                                        cellTooltip: 'How good they are at accurately juding players on their own team and not "falling in love" with certain players'
                                    },
                                    {
                                        name: 'JudgingPot',
                                        displayName: 'Judg. Pot',
                                        width: '7%',
                                        cellTooltip: 'How good they are at judging a player\'s potential'
                                    },
                                    {
                                        name: 'OffPhil',
                                        displayName: 'Off Phil.',
                ...
                angular.forEach(DB.load.data.Teams, function(value) {
                  console.log(value.TeamLogoPath)
                    vm.sides.push({
                        image: "../img/logos/" + value.TeamLogoPath.slice(0,-3) + "png",
                        teamID: value.TeamID,
